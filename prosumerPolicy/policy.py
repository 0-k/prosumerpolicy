...
class Policy:
    ...
    def _set_policy_parameters_from_file(self, path=None):
        ...
        logging.info("Loaded Policy Parameters Set From {}".format(path))
        ...
        self.__flatRateElectricityTariff = float(parameters_Policy['flatRateElectricityTariff'])  # eur/kWh
        self.electricityWholesale = float(parameters_Policy['electricityWholesale'])  # eur/kWh
        self.taxes = float(parameters_Policy['taxes'])  # eur/KWh Konzessionsabgabe & Stromsteuer
        self._componentLevyFIT = float(parameters_Policy['componentLevyFIT'])
        if self._isFixedNetworkCharges:
            self.networkCharge = float(parameters_Policy['capacityCase']['networkCharge'])
            self.fixedCapacity = float(parameters_Policy['capacityCase']['fixedCapacity']) * parameters_Economic['VAT']
            logging.info("No Network Charges Imposed")
        else:
            self.networkCharge = float(parameters_Policy['volumetricCase']['networkCharge'])  # ct per kWh
            self.fixedCapacity = float(parameters_Policy['volumetricCase']['fixedCapacity']) * parameters_Economic['VAT']   # eur per KW per year
    ...
    def _calculate_electricity_constant(self):
        ...
        c = (self.electricityWholesale * sum(totalAvgLoad) - np.dot(totalAvgLoad, totalPrice)) / sum(totalAvgLoad) #constant added of price
        return c
    ...
    def _calculate_eeg_ratio(self):
        ...
        alpha = self._componentLevyFIT * sum(totalAvgLoad) / np.dot((totalPrice + c), totalAvgLoad)
        return alpha
    ...
    def _calculate_feed_in_tariff(self):
        if self.isVFIT:
            ...
            self.__FIT = self.__beta * (self._InputSetter.get_price_list() + self.__c)  # coefficient obtained by dividing total feed in remuneration to realtime prices times production
    ...
    def _calculate_retail_electricity_prices(self):
        ...
        if self.isVFIT and self.isRTP:
            self._componentLevyFIT = self.__alpha * (self._InputSetter.get_price_list() + self.__c)  # 2.3 calculated by dividing total eeg umlage by realtime prices and load

        if self.isFixedNetworkCharges:
            self.networkCharge = float(parameters_Policy['capacityCase']['networkCharge'])
            self.fixedCapacity = float(parameters_Policy['capacityCase']['fixedCapacity']) * parameters['Economics']['VAT']
            logging.info("Capacity Network Charges Imposed")
        else:
            self.networkCharge = float(parameters_Policy['volumetricCase']['networkCharge'])  # ct per kWh
            self.fixedCapacity = float(parameters_Policy['volumetricCase']['fixedCapacity']) * parameters['Economics']['VAT']   # eur per KW per year
            logging.info("Volumetric Network Charges Imposed")

        if self.isRTP:
            self.electricityBasePrice = (self._InputSetter.get_price_list() + self.__c)
            logging.info("Real Time Pricing for Day {} and Time Duration {} Set ".format(self._InputSetter.day, self._InputSetter.timeDuration))
            ...
            logging.info("Constant Prices for day {} and Time Duration {}".format(self._InputSetter.day, self._InputSetter.timeDuration))

        total = self._componentLevyFIT + self.taxes + self.electricityBasePrice + self.networkCharge  # networkCharge is zero if case is capacity only
